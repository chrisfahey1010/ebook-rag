...
- partial-answer repair that can trim or rewrite mixed-quality generated answers down to only verified supported claims before falling back to `unsupported`
...
- the answer verifier now combines a pre-answer unsupported-classification stage with claim-level heuristic checks, optional local-model verification, partial-answer repair, and a stricter final acceptance gate for constrained numeric/date questions, but the harder suites still need benchmark reruns before those thresholds should be treated as settled
