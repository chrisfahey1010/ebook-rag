...
- the answer verifier now combines a pre-answer unsupported-classification stage with claim-level heuristic checks, optional local-model verification, and partial-answer repair, but the repaired-answer path still needs benchmark-driven tuning on harder suites
