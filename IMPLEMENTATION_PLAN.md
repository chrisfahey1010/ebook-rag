...
- retrieval quality is now in a satisfactory V1 state on the current benchmark set:
  ...
  - the latest QA pass improved structured-numeric citation evidence coverage on that Amazon benchmark, fixed the prior `net sales` versus `operating income` guidance confusion, and closed the exploratory employee-count row on page 13 by recovering split same-page table rows
  - the remaining Amazon-specific miss is now the exploratory page-11 free-cash-flow table-row citation, which still loses to the narrative explanation on page 2, so the next quality pass should stay in the QA layer rather than broad retrieval changes
...
1. close the remaining exploratory financial-row citation miss on the Amazon benchmark
   - keep using `amazon_earnings_eval.json` as the regression harness for earnings-release-style documents
   - the extractive scorer now distinguishes sibling metrics on the same page, can recover period headers from neighboring same-page chunks, and prefers explicit metric-plus-period table spans when they are available
   - the employee-count row on page 13 now passes in the real extracted document path; the remaining QA-layer miss is the free-cash-flow table row on page 11, where the answer/citation path still prefers the narrative page-2 explanation
...
3. keep broadening the benchmark set with more real-document fixtures
...
The chunking decision itself is now benchmark-backed for the current fixture set, normalization now preserves heading blocks while collapsing soft-wrapped body lines more cleanly, chunk provenance now includes character-span offsets for page-local inspection, and the benchmark workflow now distinguishes page-level citation success from excerpt-level citation accuracy. The latest split-row structured-table QA pass improved the Amazon earnings benchmark’s `answer_match_rate` to `0.8`, `citation_evidence_hit_rate` to `0.8`, `support_accuracy` to `1.0`, and `unsupported_precision` to `1.0` on the March 8, 2026 run, while holding `gating_citation_evidence_hit_rate` at `0.875`. The exploratory employee-count row now passes in the real extracted document path, which confirms that retrieval was already sufficient and the main remaining issue is citation granularity for the exploratory free-cash-flow table row on page 11. The next step should stay focused on QA extraction and citation selection for those financial-table passages, while continuing to graduate stable excerpt-level checks into the regression lane and expand real-document benchmark breadth.
